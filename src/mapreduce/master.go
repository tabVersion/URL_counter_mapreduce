...
func (mr *Master) forwardRegistrations(ch chan string) {
	i := 0
	debug("get in function forwardRegistration\n")
	for {
		mr.Lock()
		debug("current number of idle machines %d\n", i)
		if len(mr.workers) > i {
			// there's a worker that we haven't told schedule() about.
			w := mr.workers[i]
			debug("send %v to work", w)
...
func Distributed(jobName string, files []string, nreduce int, master string) (mr *Master) {
	mr = newMaster(master)
	mr.startRPCServer()
	debug("rpc service on\n")
	go mr.run(jobName, files, nreduce,
		func(phase jobPhase) {
			ch := make(chan string)
			go mr.forwardRegistrations(ch)
			debug("current port is %v\n", ch)
...
func GoDistributed(jobName string, files []string, nReduce int, master string,
	MapFunc func(string, string) []KeyValue,
	ReduceFunc func(string, []string) string,) (mr *Master) {
	mr = Distributed(jobName, files, nReduce, master)
	for i := 0; i < nReduce; i++ {
		go RunWorker(mr.address, "worker"+strconv.Itoa(i),
